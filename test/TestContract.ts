  ...
  describe("Methods", function () {
    describe("deposit", function () {
      it("should return EtherLessThanRequired", async function () {
        ...
        await expect(lock.deposit(val, { value: 10 ** 7 }))
          .to.be.revertedWithCustomError(lock, "EtherLessThanRequired")
      ...
      it("should return InsufficientBalance", async function () {
        ...
        lock.deposit(val, { value: 10 ** 14 });

        await expect(
          lock.connect(otherAccount).deposit(val, { value: 10 ** 14 })
        )
          .to.be.revertedWithCustomError(lock, "InsufficientBalance")
          .withArgs(otherAccount, 0, val);
      });
    ...
    describe("transfer", function () {
      it("should return InsufficientBalance", async function () {
        ...
        await expect(lock.transfer(otherAccount, val))
          .to.be.revertedWithCustomError(lock, "InsufficientBalance")
      ...
      it("should transfer success", async function () {
        ...
        await expect(lock.transfer(otherAccount, val)).to.changeTokenBalances(
          lock,
          [owner, otherAccount],
          [-val, val]
        );
